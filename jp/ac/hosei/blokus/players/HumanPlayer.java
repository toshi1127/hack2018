		...
		while(true) {
			...
			if(x >= boardLeft && x < boardRight && y >= boardTop && y < boardBottom) {
				...
				if(pieceId >= 0 && isValid(available, figure, ii, jj)) {
					...
					viewer.drawBoard();
					drawFigure(figure, id, boardLeft + jj * cellSize, boardLeft + ii * cellSize, true);
					drawFigure(figure, id, pieceLeft, pieceTop, false);
					drawButtons();
				}

			} else if(x >= boardLeft - extraWidth && x < boardRight + extraWidth
					&& y >= boardBottom + margin && y < boardBottom + margin + cellSize * 12) {
				// selection board
				...
				int ii = (y - (boardBottom + margin)) / cellSize;
				int jj = (x - (boardLeft - extraWidth)) / cellSize;
